...
//
// This is to deal with infamous “Could not decode a text frame as UTF-8″
// issue. Taken from:
// http://dougal.gunters.org/blog/2012/03/14/dealing-with-utf-in-node-js/
//
function _utfSafe(str) {
...
function getTweetText(tweet) {
    ...
    return _utfSafe(escapeText(text))
}
...
function sortTweets(args) {
    ...
    for (; i < length; i++) {
        ...
        tweet = args.tweets[i]
        tweet.text = getTweetText(tweet)
        tag = getTagForTweet(tweet, args.user)
        tweet.user.screen_name = tweet.user.screen_name.toString('utf8')
        tweet.user.name = tweet.user.name.toString('utf8')

        delete tweet.truncated
