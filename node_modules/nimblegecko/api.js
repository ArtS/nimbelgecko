    ...
    ng.db.getUserById({
        ...
        next: function(err, user) {
            ...
            if (user === null || typeof user === "undefined") {
                ...
                ng.log.log('New user logged in, creating profile...')
                ...
                ng.log.log('Exisiting user logged in, updating data...')

                ng.api.updateUserDataAndCheckTweets({
                    oauth_token: opts.oauth_token,
                    oauth_token_secret: opts.oauth_token_secret,
...
exports.getLatestTweetsFromTwitter = function(opts) {
    ...
    function _retrieveBunch(next) {
        ...
        var o = {
            oauth_token: opts.user.oauth_token,
            oauth_token_secret: opts.user.oauth_token_secret,
            next: _isEndReached,
            params: {
                count: 200
            }
...
exports.getAndSaveLatestTweetsFromTwitter = function(user) {
    ...
    function _tryToGetTweetsInternal() {
        ...
        ng.api.getLatestTweetsFromTwitter({
            ...
            next: function(err, tweets) {
                if (err) {
                    ...
                    // leaving it for now
                    //
                    // After some investigation it seems that twitter returns 500
                    // for new sign-ups in NG, although if you repeat that
                    // afterwards, it will possibly work.
                    debugger
                    ng.log.error(err, 'Not sure what that is')
                    if (retries < 5) {
                        ng.log.log('Retrying to get tweets again for user with no tweets...')
                        retries++
                        setTimeout(_tryToGetTweetsInternal, 1000 * (retries + 1))
                    } else {
                        ng.log.log('Exhausted all teh retry attemps, giving up...')
                    }
                    return
                }
...
exports.updateUserDataAndCheckTweets = function(opts) {
        ...
        next: function(err, user) {
            ...
            ng.db.getUserTweetsCount({
                ...
                next: function(err, count) {
                    ...
                    if (count !== 0) {
                        ng.log.log('User @' + opts.user.screen_name +
                                   ' logged in with ' + count + ' tweets, no need to try to retrieve some from Twitter...')
                        return
                    }
                    ...
                    ng.api.getAndSaveLatestTweetsFromTwitter(opts.user)
                }
            })
