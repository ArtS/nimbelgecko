...
exports.updateExistingUser = function(opts) {
    ...
    opts.user.screen_name = opts.data.screen_name
    opts.user.oauth_token = opts.oauth_token
    opts.user.oauth_token_secret = opts.oauth_token_secret
...
exports.saveStreamItem = function(item) {
    ...
    for_user = (typeof item.for_user === 'string' ? item.for_user :
...
exports.initDatabase = function(onDatabaseReady) {
    ...
    if (!Array.isArray(ng.conf.collections)) {
        throw {
    ...
    var colNames = ng.conf.collections.splice(0)

    colNames.push(ng.conf.sessionStoreName)

    function initIndexes() {
        //
        // In theory this should create indexes only once, when DB is empty
        //
        collections[TWEETS_COLLECTION].ensureIndex({for_user: 1})
        collections[TWEETS_COLLECTION].ensureIndex({id: 1})
    }
...
exports.getMongoStore = function() {
    ...
    if (db === null)
        throw {
           name: 'DBNotInitialisedException',
           message: 'Database is not initialised. Call initDatabase first',
	   stack: new Error().stack
       }
...
exports.getUserTweetsCount = function getUserTweetsCount(opts) {
    ng.utils.checkRequiredOptions(opts, ['user_id', 'next'])
    collections[TWEETS_COLLECTION].count({'for_user': opts.user_id.toString()}, opts.next)
}
